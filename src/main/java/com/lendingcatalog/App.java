...
import static com.lendingcatalog.util.FileStorageService.readContentsOfFile;
...
public class App {
    ...
    private void initialize() {
        ...
        for (Member m : buildMemberList()) {
            String memberFile = FILE_BASE_PATH + returnMemberFile(m.getFirstName());
            buildMemberMap(m, memberFile);
        }

    }

    // Create list of Member objects by looping through member files & retrieving first and last name
    private List<Member> buildMemberList() {
        List<Member> memberList = new ArrayList<>();
        try {
            List<String> contentsOfFile = readContentsOfFile(FILE_BASE_PATH + "members.dat");
            for (String s : contentsOfFile) {
                String[] fileLine = s.split(FIELD_DELIMITER);
                Member member = new Member(fileLine[0], fileLine[1]);
    ...
    // Get appropriate member file name by searching through member file for a first name match
    private String returnMemberFile(String firstName) {
        String memberFile = "";
        try {
            List<String> contentsOfFile = readContentsOfFile(FILE_BASE_PATH + "members.dat");
            for (String s : contentsOfFile) {
                String[] fileLine = s.split(FIELD_DELIMITER);
                if (firstName.equalsIgnoreCase(fileLine[0])) {
                    memberFile = fileLine[2];
                }
            }
        } catch (FileStorageException e) {
            e.printStackTrace();
    ...
    private void buildMemberMap(Member member, String fileName) {
        List<CatalogItem> memberData = new ArrayList<>();

        List<String> contentsOfFile = null;
        try {
            contentsOfFile = FileStorageService.readContentsOfFile(fileName);
        } catch (FileStorageException e) {
            e.printStackTrace();
        }
        {
            assert contentsOfFile != null;
            for (String s : contentsOfFile) {
                String[] lineSplit = s.split(FIELD_DELIMITER);
                if (lineSplit.length != 4) {
                    continue;
                }
                if (!(lineSplit[0].equalsIgnoreCase("book") ||
                        lineSplit[0].equalsIgnoreCase("movie") ||
                        lineSplit[0].equalsIgnoreCase("tool"))) {
                    continue;
                }
                memberData.add(createItem(lineSplit[0], lineSplit[1], lineSplit[2], lineSplit[3]));
            }
            catalog.put(member.toString(), memberData);
        }
    }

    // Instantiate member items
    private CatalogItem createItem(String itemType, String title, String creator, String yearOrCount) {
        CatalogItem item = null;
        if (itemType.equalsIgnoreCase("book")) {
            LocalDate date = LocalDate.parse(yearOrCount);
            item = new Book(title, creator, date);

        }
        else if (itemType.equalsIgnoreCase("movie")) {
            LocalDate date = LocalDate.parse(yearOrCount);
            item = new Movie(title, creator, date);

        }
        else if (itemType.equalsIgnoreCase("tool")) {
            int toolCount = Integer.parseInt(yearOrCount);
            item = new Tool(title, creator, toolCount);
        }
        if (item != null) {
            item.registerItem();
        }
